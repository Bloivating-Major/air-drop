...
// Add type definitions for ImageKit responses
interface ImageKitFileObject {
  fileId: string;
  name: string;
  // other properties...
}
          ...
          if (imagekitFileId) {
            try {
              ...
              if (searchResults && searchResults.length > 0) {
                // Check if the result has a fileId property
                const result = searchResults[0] as ImageKitFileObject;
                if ('fileId' in result) {
                  await imagekit.deleteFile(result.fileId);
                } else {
                  console.log("Search result doesn't have fileId:", result);
                  // Try to delete using the imagekitFileId as fallback
